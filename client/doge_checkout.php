...
if (isset($_POST['submit'])) {
    ...
    $check_cart = $pdo->prepare("SELECT * FROM cart WHERE dogeuser_id = ?");
    ...
    if ($check_cart->rowCount() > 0) {
        ...
        if ($address == '') {
            ...
            $insert_order = $pdo->prepare("INSERT INTO orders(dogeuser_id, name, number, email, method, address, total_products, total_price) VALUES(?,?,?,?,?,?,?,?)");
            $insert_order->execute([$dogeuser_id, $name, $number, $email, $method, $address, $total_products, $total_price]);

            $delete_cart = $pdo->prepare("DELETE FROM cart WHERE dogeuser_id = ?");
